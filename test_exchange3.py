...
Tests performed (in order):
  ...
  2. DNS / network        — Exchange hostname resolves and EWS_PORT is open
...
EWS_PORT         = 444                      # default 443; change if your server
                                            # uses a non-standard port (e.g. 444)
...
# Derived — full EWS endpoint URL built from server + port
EWS_ENDPOINT = (
    f"https://{EXCHANGE_SERVER}/EWS/Exchange.asmx"
    if EWS_PORT == 443
    else f"https://{EXCHANGE_SERVER}:{EWS_PORT}/EWS/Exchange.asmx"
)
...
print(f"{INFO} EWS endpoint: {EWS_ENDPOINT}")
...
if ip:
    try:
        sock = socket.create_connection((EXCHANGE_SERVER, EWS_PORT), timeout=5)
        sock.close()
        log(PASS, f"TCP port {EWS_PORT} open")
    except (socket.timeout, OSError) as e:
        log(FAIL, f"TCP port {EWS_PORT} unreachable", str(e))
...
try:
    ...
    req = urllib.request.Request(EWS_ENDPOINT, method="GET")
    try:
...
print(f"{INFO} Using service_endpoint: {EWS_ENDPOINT}")
...
for auth_type, auth_label in auth_types:
    ...
    for uname, uname_label in username_formats:
        ...
        try:
            ...
            # Use service_endpoint (full URL) so non-standard ports are respected.
            # Using `server=` alone always assumes port 443.
...
if winning_combo:
    print(f"\n  Working combination:")
    print(f"    service_endpoint : {EWS_ENDPOINT}")
