...
Original file is located at
    https://colab.research.google.com/github/SunSlick2/emailSort/blob/main/Email_Sorter_Python_Application_(Version_38_13_ResearchEmail_Sender_Only).ipynb
"""
...
class EmailSorter:
    ...
    def save_smtp_cache(self):
        """
        Saves newly resolved SMTP entries to the separate SMTP cache Excel file.
        Preserves MSIP labels and all workbook properties by using openpyxl with
        read-only mode disabled and preserving VBA/properties.
        ...
        while not saved_successfully:
            try:
                ...
                file_exists = os.path.exists(self.smtp_cache_path)
                
                if file_exists:
                    # Load workbook with keep_vba=True to preserve macros and MSIP labels
                    wb = openpyxl_load_workbook(self.smtp_cache_path, keep_vba=True, data_only=False)
                else:
                    # Create new workbook
                    wb = openpyxl.Workbook()
                    # Remove default sheet if we're creating a new one
                ...
                if cache_sheet_name not in wb.sheetnames:
                    ...
                    print(f"Created new sheet '{cache_sheet_name}' in SMTP cache file.")
                else:
                    ...
                    if ws.max_row == 0 or (ws.cell(row=1, column=1).value not in ['EntryName', 'entryname', 'EntryName']):
                        ...
                        print(f"Added headers to existing sheet '{cache_sheet_name}' in SMTP cache file.")
                ...
                if entries_added > 0:
                    # Save with keep_vba=True to preserve MSIP labels and all properties
                    wb.save(self.smtp_cache_path)
                    print(f"Saved {entries_added} new SMTP entries to '{cache_sheet_name}' in '{self.smtp_cache_path}'.")
                    self.invalid_logger.info(f"SMTPCacheSave|save_smtp_cache|Saved {entries_added} new entries to cache file.")
                else:
                ...
                error_msg = f"Failed to save SMTP cache: Cache file is currently open. Error: {e}"
                ...
                retry_choice = messagebox.askokcancel(
                    "SMTP Cache File Locked",
                    f"The SMTP cache file '{os.path.basename(self.smtp_cache_path)}' is currently open.\n"
