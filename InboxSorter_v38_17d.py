# -*- coding: utf-8 -*-
"""Email_Sorter_Python_Application_(Version_38_13_ResearchEmail_Sender_Only).ipynb
...
from openpyxl.reader.excel import load_workbook as openpyxl_load_workbook
...
class EmailSorter:
    ...
    Version 38.13: Modified ResearchEmail rule (Rule 8) to only check "From" (sender) field
                  Added separate SMTP cache file with MSIP label preservation
    Version 38.17d: Fixed MSIP label not recognised by Office after save — openpyxl silently
                   drops the classification relationship from _rels/.rels; added
                   _ensure_label_relationship() to restore it on every inject pass
    """
    ...
    def extract_sender_address_only(self, outlook_namespace, mail):
        ...
        return set([sender_address]) if sender_address else set()
    ...
    def process_email(self, outlook_namespace, mail, logger, folder_objects_map):
        ...
        """
        try:
        # NEW: Skip processing if this mail is currently open in an Inspector
    ...
    def _ensure_label_relationship(xml_text: str) -> str:
        ...
        openpyxl silently drops this entry on save, which causes Office to
        ignore LabelInfo.xml even when the file is physically present in the zip.
        ...
        with zipfile.ZipFile(src_path, "r") as zin, \
            ...
            for item in zin.infolist():
                ...
                if item.filename == EmailSorter.LABEL_PART_PATH:
                    ...
                    # openpyxl drops it, so Office won't recognise the label
    ...
    def save_smtp_cache(self) -> None:
        ...
        cache_sheet_name  = self.config["sheet_map"]["SMTPResolutionCache"]["sheet"]
        txt_fallback_dir  = self.config['smtp_fallback_dir']
        ...
        while not saved_successfully:
            try:
                ...
                self._inject_msip_label(tmp_path, tmp_path2)  # read from tmp, write labelled to tmp2
    ...
    def start_gui(self):
        ...
        root = tk.Tk()
        root.title("Email Sorter v38.17d")
